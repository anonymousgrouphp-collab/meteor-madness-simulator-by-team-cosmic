document.addEventListener('DOMContentLoaded', () => {
    ...
    function generateImpactBriefing() {
        ...
        const briefingHTML = `
            ...
            <p>Analysis of a simulated impact event involving a Near-Earth Object (NEO) with a diameter of <strong>${diameter} m</strong> and an impact velocity of <strong>${velocity} km/s</strong> targeting the <strong>${location.name}</strong> region.</p>
            ...
            <p>The energy released is approximately <strong>${(impactEnergy / 15).toFixed(1)} times</strong> more powerful than the 1908 Tunguska event.</p>
        `;
    ...
    function showReportCard() {
        ...
        dom.reportCardContent.innerHTML = `
            ...
            <p><strong>Target Region:</strong> ${appState.location.name}</p>
            ...
            <p><strong>Resulting Energy:</strong> ${Number(energy).toLocaleString()} MT</p>
        `;
    ...
    async function initMap() {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`GeoJSON file not found: ${response.statusText}`);
            }
            const data = await response.json();
            ...
            console.error("Could not load GeoJSON data. This is a critical error. Please ensure 'countries.geojson' is in the '/data/' folder of your repository.", error);
            dom.locationName.textContent = "Map Data Failed to Load";
            // Fallback if the geojson fails to load
            map.on('click', onOceanClick); 
        }
    }
    ...
    function onOceanClick(e) {
        if(e.originalEvent && e.originalEvent.target !== map.getContainer()) return;
